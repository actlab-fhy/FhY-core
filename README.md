...
| Solver                                   | `fhy_core.symbolic.solver`, the entry point for symbolic queries: `simplify_expression` (SymPy-backed), plus `check_expression_satisfiability`, `does_expression_imply`, and `holds_for_all_free_assignments` (Z3-backed), each returning `True`/`False`/`None` where `None` means the solver returned unknown. `assert_expression_implies` and `assert_holds_for_all_free_assignments` raise `UndecidableError` instead of returning `None`. Backend selection is explicit via `SolverBackend`; an unsupported (backend, query) pairing raises `SolverCapabilityError`, and `get_backend_capabilities` reports what each backend can answer. |
