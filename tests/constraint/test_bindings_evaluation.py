...
def test_equation_constraint_bindings_logs_debug_when_free_identifiers_unbound(
    ...
    with caplog.at_level(logging.DEBUG, logger="fhy_core.constraint"):
        outcome = constraint.evaluate_with_bindings({x: 3})

    assert outcome is ConstraintOutcome.UNDECIDED
    assert any(
        record.levelno == logging.DEBUG and record.name == "fhy_core.constraint"
        for record in caplog.records
    ), "expected a DEBUG-level log record from fhy_core.constraint"
...
def test_equation_constraint_bindings_logs_warning_when_fully_bound_but_irreducible(
    ...
    assert outcome is ConstraintOutcome.UNDECIDED
    assert any(
        record.levelno == logging.WARNING and record.name == "fhy_core.constraint"
        for record in caplog.records
