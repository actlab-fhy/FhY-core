...
`INTEGER_RESULT_NATIVE_FUNCTIONS`. The tree is never evaluated, so an
...
def build_structural_expression_strategy(
    ...
    include ``DIVIDE`` and ``POWER``, and calls reach any registered
    function whose declared sorts fit the position. The tree is never
