...
class FrozenMutationError(AttributeError):
    ...
    standard ``__setattr__``/``__delattr__`` convention (including code
    that previously caught :class:`dataclasses.FrozenInstanceError`) keep
    working without explicit knowledge of FhY's error type.
...
class FrozenMixin(ABC):
    ...
    def __new__(cls: type[_FrozenMixinT], *args: Any, **kwargs: Any) -> _FrozenMixinT:
        del args, kwargs
